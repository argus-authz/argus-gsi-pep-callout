...
static int get_token(int fd, unsigned char ** token, size_t * token_length);

static int put_token(int fd, unsigned char * token, size_t token_length);

static OM_uint32 accept_sec_context(int client_fd, char ** name,
		gss_ctx_id_t * context, gss_cred_id_t * delegated_cred,
		gss_cred_id_t credential);

static OM_uint32 init_sec_context(int client_fd, gss_cred_id_t credential,
		gss_ctx_id_t * context);

gss_cred_id_t globus_gsi_gssapi_test_acquire_credential() {
...
void globus_gsi_gssapi_test_release_credential(gss_cred_id_t * credential) {
	OM_uint32 minor_status;
	OM_uint32 major_status = GSS_S_COMPLETE;
...
globus_bool_t globus_gsi_gssapi_test_authenticate(int fd, globus_bool_t server,
		gss_cred_id_t credential, gss_ctx_id_t * context_handle,
		char ** user_id, gss_cred_id_t * delegated_cred) {
	OM_uint32 major_status = GSS_S_COMPLETE;

	if (server == GLOBUS_TRUE) {
		major_status = accept_sec_context(fd, user_id, context_handle,
				delegated_cred, credential);
	} else {
		major_status = init_sec_context(fd, credential, context_handle);
	}

	return major_status == GSS_S_COMPLETE ? GLOBUS_TRUE : GLOBUS_FALSE;
}

void globus_gsi_gssapi_test_cleanup(gss_ctx_id_t * context_handle,
		char * userid, gss_cred_id_t * delegated_cred) {
	...
	major_status = gss_delete_sec_context(&minor_status, context_handle,
			GSS_C_NO_BUFFER);

	if (delegated_cred != GSS_C_NO_CREDENTIAL) {
		major_status = gss_release_cred(&minor_status, delegated_cred);
	}
}

globus_bool_t globus_gsi_gssapi_test_export_context(char * filename,
		gss_ctx_id_t * context) {
	OM_uint32 major_status;
	OM_uint32 minor_status;
	...
	major_status = gss_export_sec_context(&minor_status, context,
...
globus_bool_t globus_gsi_gssapi_test_import_context(char * filename,
		gss_ctx_id_t * context) {
	OM_uint32 major_status;
	OM_uint32 minor_status;
	globus_bool_t result = GLOBUS_TRUE;
	...
	major_status = gss_import_sec_context(&minor_status,
			(gss_buffer_t) &import_token, context);
...
globus_bool_t globus_gsi_gssapi_test_send_hello(int fd, gss_ctx_id_t context) {
	OM_uint32 major_status;
	OM_uint32 minor_status;
	globus_bool_t result = GLOBUS_TRUE;
	...
	major_status = gss_wrap(&minor_status, context, 0, GSS_C_QOP_DEFAULT,
			(gss_buffer_t) &input_token, NULL, (gss_buffer_t) &send_token);
	...
	while (send_token.length - written
			&& (rc = write(fd, &((char *) send_token.value)[written],
					send_token.length - written)) > 0 && (written += rc))
		;
...
globus_bool_t globus_gsi_gssapi_test_receive_hello(int fd, gss_ctx_id_t context) {
	OM_uint32 major_status;
	OM_uint32 minor_status;
	globus_bool_t result = GLOBUS_TRUE;
	char buffer[128];
	...
	major_status = gss_unwrap(&minor_status, context,
			(gss_buffer_t) &recv_token, (gss_buffer_t) &output_token, NULL,
			NULL);
	if (GSS_ERROR(major_status))
	{
		char * error_str;
...
globus_bool_t globus_gsi_gssapi_test_dump_cert_chain(char * filename,
		gss_ctx_id_t context) {
	OM_uint32 major_status;
	OM_uint32 minor_status;
	int i;
	globus_bool_t result = GLOBUS_TRUE;
	FILE * dump_file;
	gss_OID_desc cert_chain_oid = { 11,
			"\x2b\x06\x01\x04\x01\x9b\x50\x01\x01\x01\x08" };
	...
	major_status = gss_inquire_sec_context_by_oid(&minor_status, context,
			&cert_chain_oid, &cert_chain_buffers);
	...
	for (i = 0; i < cert_chain_buffers->count; i++) {
		...
		cert = d2i_X509(NULL, &tmp_ptr, cert_chain_buffers->elements[i].length);
		if (cert == NULL) {
...
static int get_token(int fd, unsigned char ** token, size_t * token_length) {
	...
	while (num_read < 4) {
		n_read = read(fd, token_length_buffer + num_read, 4 - num_read);
	...
	*token = malloc(*token_length);
	...
	while (num_read < *token_length) {
		n_read = read(fd, *token + num_read, *token_length - num_read);
...
static int put_token(int fd, unsigned char * token, size_t token_length) {
	...
	while (num_written < 4) {
		n_written = write(fd, token_length_buffer + num_written, 4
				- num_written);
		if (n_written < 0) {
			if (errno== EINTR) {
				continue;
	...
	while (num_written < token_length) {
		n_written = write(fd, token + num_written, token_length - num_written);
		if (n_written < 0) {
			if (errno== EINTR) {
				continue;
			} else {
...
static OM_uint32 accept_sec_context(int client_fd, char ** name,
	...
	do {
		token_status = get_token(client_fd, &input_token.value,
				&input_token.length);
		if (token_status != 0) {
		...
		major_status = gss_accept_sec_context(&minor_status, context,
				credential, &input_token, GSS_C_NO_CHANNEL_BINDINGS,
				&client_name, &mech_type, &output_token, &ret_flags, &time_ret,
				delegated_cred);

		if (major_status != GSS_S_COMPLETE && major_status
		...
		if (output_token.length != 0) {
			token_status = put_token(client_fd, output_token.value,
					output_token.length);
			if (token_status != 0) {
	...
	major_status = gss_display_name(&minor_status, client_name, &name_buffer,
			NULL);
...
static OM_uint32 init_sec_context(int client_fd, gss_cred_id_t credential,
	...
	major_status = gss_inquire_cred(&minor_status, credential, &target_name,
			NULL, NULL, NULL);
	...
	while (!context_established) {
		major_status = gss_init_sec_context(&minor_status, credential, context,
				target_name, GSS_C_NO_OID, /*mech_type*/
				req_flags, 0, /* default time */
				GSS_C_NO_CHANNEL_BINDINGS, &input_token, actual_mech_type,
				&output_token, &ret_flags, &time_ret);
		if (major_status != GSS_S_COMPLETE && major_status
				!= GSS_S_CONTINUE_NEEDED) {
			char * error_string = NULL;
			globus_object_t * error_obj = NULL;
